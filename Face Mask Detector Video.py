...
maskNet = load_model(facemaskDetectorPath)
...
while True:
    ...
    # Fliping the frame to have mirror-like video
    frame_flip = cv2.flip(frame, 1)

    # detect faces in the frame and determine if they are wearing a
    # face mask or not
    (locs, preds) = detect_and_predict_mask(frame_flip, faceNet, maskNet)
    ...
    for (box, pred) in zip(locs, preds):
        ...
        # frame
        cv2.putText(frame_flip, label, (startX, startY - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 2)
        cv2.rectangle(frame_flip, (startX, startY), (endX, endY), color, 2)
